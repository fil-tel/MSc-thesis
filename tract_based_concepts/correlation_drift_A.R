...
tracts <- ts_tracts(ts, census = 66000, squashed = TRUE)
...
# get_df_2_plot <- function(bin_mat, pairs_df){
#   bin_mat[bin_mat==0] <- -1  
#   pairs_df <- pairs_df %>% rowwise() %>% mutate(cor = (bin_mat[,c(x, y)][rowSums(bin_mat[,c(x, y)])!=-2,][,1]%*%bin_mat[,c(x, y)][rowSums(bin_mat[,c(x, y)])!=-2,][,2])/nrow(bin_mat[,c(x, y)][rowSums(bin_mat[,c(x, y)])!=-2,])) %>% ungroup()
# 
#   # get matrix for f3
#   mat_f3 <- long_to_mat(pairs_df, "x", "y", "f3")
#   # get matrix for tracts corrr
