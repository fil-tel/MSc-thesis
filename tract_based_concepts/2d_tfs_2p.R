...
# for two population that experience two pulses of introgression and
...
saveRDS(df_subset, file = "df_subset_2p.rds")
df_subset <- readRDS("df_subset_2p.rds")

p_subset <- ggplot(df_subset, aes(x = col, y = row, fill = Count)) +
  geom_tile() +
  scale_fill_viridis_c() +
  facet_wrap(.~time, labeller = labeller(time = my_labeller)) +
  labs(x = "EUR_1", y = "EUR_2")+theme(aspect.ratio=1)+ggtitle("Model Y - sub-tracts approach")

ggsave("figs/2d_tfs_2p_subset.png", plot = p_subset, width = 190, height = 143, units = "mm")
...
saveRDS(df_sites, file = "df_sites_2p.rds")
df_sites <- readRDS("df_sites_2p.rds")

p_sites <- ggplot(df_sites, aes(x = col, y = row, fill = Count)) +
  geom_tile() +
  scale_fill_viridis_c() +
  facet_wrap(.~time, labeller = labeller(time = my_labeller)) +
  labs(x = "EUR_1", y = "EUR_2")+theme(aspect.ratio=1)+ggtitle("Model Y - recombination sites approach")

ggsave("figs/2d_tfs_2p_sites.png", plot = p_sites, width = 190, height = 143, units = "mm")
...
saveRDS(df_wind, file = "df_wind_2p.rds")
df_wind <- readRDS("df_wind_2p.rds")

p_wind <- ggplot(df_wind, aes(x = col, y = row, fill = Count)) +
  geom_tile() +
  scale_fill_viridis_c() +
  facet_wrap(.~time, labeller = labeller(time = my_labeller)) +
  labs(x = "EUR_1", y = "EUR_2")+theme(aspect.ratio=1)+ggtitle("Model Y - windows approach")

ggsave("figs/2d_tfs_2p_wind.png", plot = p_wind, width = 190, height = 143, units = "mm")
...
saveRDS(df_uni, file = "df_uni_2p.rds")
df_uni <- readRDS("df_uni_2p.rds")

p_uni <- ggplot(df_uni, aes(x = col, y = row, fill = Count)) +
  geom_tile() +
  scale_fill_viridis_c() +
  facet_wrap(.~time, labeller = labeller(time = my_labeller)) +
  labs(x = "EUR_1", y = "EUR_2")+theme(aspect.ratio=1)+ggtitle("Model Y - uniqueness approach")

ggsave("figs/2d_tfs_2p_uni.png", plot = p_uni, width = 190, height = 143, units = "mm")

plot_list <- list(uni = p_uni, wind= p_wind, sub=p_subset, sites=p_sites)
saveRDS(plot_list, "plots_2p.rds")
# grid_int <- cowplot::plot_grid(p_wind, p_subset, ncol = 1, labels = "AUTO")
# ggsave(filename = "figs/test.png", plot = grid_int, width = 10, height = 13)
