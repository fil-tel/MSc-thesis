...
read_tracts_all <- function(set = NULL) {
...
plot_tfs <- function(tfs){
  ggplot(data.frame(tfs))+geom_bar(stat="identity", aes(seq_along(tfs),tfs))+theme_minimal()+xlab("Introgressed tracts count")+ylab("Proportion of tracts")
}
...
get_bin_mat_sites_emp <- function(tracts_gr){
  ...
  bin_mat_l <- lapply(as.character(unique(seqnames(tracts_gr))),
                      function(chrom) {
                        chrom_gr <- filter(tracts_gr, seqnames==chrom)
                        mat <- get_bin_mat_sites(chrom_gr)
                        # This is because the some inds might not have tracts 
                        # for some chromosomes and in order to create one big matrix it is important
                        # to have the dimensions consistent
                        full_matrix <- matrix(0, nrow = nrow(mat), ncol = length(col_names), dimnames = list(rownames(mat), col_names))
...
plot_tfs <- function(tfs, title, subtitle = ""){
  # title <- unlist(strsplit(title, "_"))
  p <- ggplot()+geom_bar(stat="identity", aes(seq_along(tfs),tfs/sum(tfs)))+theme_minimal()+ylab("Proportion of tracts")+xlab("Introgressed tract counts")+ggtitle(title, subtitle)
  p  
}
...
# function to get df of histogram of length of tracts
get_length_df <- function(tract_gr){
  tract_lengths <- tracts_gr$length
  h_ind <- tract_lengths %>% hist(breaks = 50, plot = FALSE)
  length_df <- data.frame(x = as.integer(h_ind$mids), y = h_ind$density, line = dexp(h_ind$mids, rate = 1 / mean(tract_lengths)))
  length_df
}