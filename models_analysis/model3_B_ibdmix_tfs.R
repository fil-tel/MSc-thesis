...
# model 1 A ---------------------------------------------------------------
if(!dir.exists("tfs_df_ibdmix")) dir.create("tfs_df_ibdmix")
if(!dir.exists("rds_files_ibdmix")) dir.create("rds_files_ibdmix")

# size to remove the tracts below a certain length
size_cutoff <- 50e3
suffix <- "3_B"
...
# subset
if (!file.exists(sprintf("rds_files_ibdmix//bin_mat_sub_%s_ibdmix.rds", suffix))) {
  bin_mat_list_sub <- mclapply(tracts_list_gr, get_bin_mat_subset, mc.cores = ncores)
  saveRDS(bin_mat_list_sub, sprintf("rds_files_ibdmix//bin_mat_sub_%s_ibdmix.rds", suffix))
} else {
  bin_mat_list_sub <- readRDS(sprintf("rds_files_ibdmix//bin_mat_sub_%s_ibdmix.rds", suffix))
}

# unique
if (!file.exists(sprintf("rds_files_ibdmix//bin_mat_uni_%s_ibdmix.rds", suffix))) {
  bin_mat_list_uni <- mclapply(tracts_list_gr, get_bin_mat_unique, mc.cores = ncores)
  saveRDS(bin_mat_list_uni, sprintf("rds_files_ibdmix//bin_mat_uni_%s_ibdmix.rds", suffix))
} else {
  bin_mat_list_uni <- readRDS(sprintf("rds_files_ibdmix/bin_mat_uni_%s_ibdmix.rds", suffix))
}

# window
if (!file.exists(sprintf("rds_files_ibdmix//bin_mat_wind_%s_ibdmix.rds", suffix))) {
  bin_mat_list_wind <- mclapply(tracts_list_gr, function(x) get_bin_mat_windows(tracts_gr = x, len_chr = 100e6), mc.cores = ncores)
  saveRDS(bin_mat_list_wind, sprintf("rds_files_ibdmix//bin_mat_wind_%s_ibdmix.rds", suffix))
} else {
  bin_mat_list_wind <- readRDS(sprintf("rds_files_ibdmix//bin_mat_wind_%s_ibdmix.rds", suffix))
}

# sites
if (!file.exists(sprintf("rds_files_ibdmix//bin_mat_sites_%s_ibdmix.rds", suffix))) {
  bin_mat_list_sites <- mclapply(tracts_list_gr, function(x) get_bin_mat_sites(tracts_gr = x), mc.cores = ncores)
  saveRDS(bin_mat_list_sites, sprintf("rds_files_ibdmix//bin_mat_sites_%s_ibdmix.rds", suffix))
} else {
  bin_mat_list_sites <- readRDS(sprintf("rds_files_ibdmix//bin_mat_sites_%s_ibdmix.rds", suffix))
}
...
get_df_2_plot <- function(bin_mat_list){
  ...
  tfs_df <- as.data.frame(sapply(bin_mat_list, function(x) compute_tfs(x, n_sample = 50)))
...
# save the df
write_tsv(df2save, file = sprintf("tfs_df_ibdmix//tfs_model_%s_ibdmix.tsv", suffix))
