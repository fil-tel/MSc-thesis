...
plot_tfs <- function(tfs, title){
  # title <- unlist(strsplit(title, "_"))
  p <- ggplot()+geom_bar(stat="identity", aes(seq_along(tfs),tfs/sum(tfs)))+theme_minimal()+ylab("Proportion of tracts")+xlab("Introgressed tract counts")+ggtitle(title)
  p  
}
...
# function to convert to gr
convert_2_gr <- function(tracts_df){
  tracts_gr <- makeGRangesFromDataFrame(
    tracts_df,
    keep.extra.columns = TRUE,
    seqnames.field = "chrom",
